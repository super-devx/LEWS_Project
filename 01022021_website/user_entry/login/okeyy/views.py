...
def opendatabase():
  try:
    connection = psycopg2.connect(user="postgres",password="Root@1234A",host="127.0.0.1",port="5432",database="backup_database")
...
def add(request):
  ...
  if chart_type == 'bar': 
    ...
    ax1.set_ylabel(l1, fontsize=7,color=color)
    if len(set_ty) ==3:  
      ax2 = ax1.twinx()
      ax3 = ax1.twinx()
    ...
    if len(set_ty)==3:
      ...
      ax2.set_ylabel(l2, fontsize=7,color=color)  # we already handled the x-label with ax1
    #ax2.plot(t, data2, color=color)
      ax3.set_ylabel(l2, fontsize=7,color=color)  # we already handled the x-label with ax1
    #plt.xlabel('DAY',rotation=0)
    #plt.ylabel('SENSOR VALUE',rotation=90)
    color_line=['red','blue','green','tab:brown','pink','silver','navy','cyan','violet']
    flag=False
    i=-1
    markers = ['<','o', 'v', 'x', 'X', 'D', '|','>', '+', '.', ',']
    for row in range(0,len(sensor_id)):
      ...
      for col in range(0,len(data)):
        if data[col][0] == sensor_id[row]:
          ...
          if sensor_id[row]=='nt_n4_pr1' or sensor_id[row]=='nt_n4_pr1_f' or sensor_id[row]=='nt_n4_pr1_r':
          ...
          if data[countd][3]=='moisture':
            q="select 100*(min_value-"+str(data[countd][1])+")/(min_value-max_value) from sensor_thresold where sensor_id in ('"+sensor_id[row]+"')"
            print(q)
            cursor.execute(q)
            temp_sensor = cursor.fetchall()
            y_temp.append([data[countd][3],temp_sensor[0][0],data[countd][2]])
            continue
      ...
      if not(flag):      
        ...
        ax1.plot(fx,fy,label=label,color=color_line[i%9],linewidth=.5,marker=markers[i%6],markersize=.1)
        #ax1.scatter(fx, fy,5)
        ax1.legend(bbox_to_anchor=(0,1.25), loc='upper center', prop=font)   
      ...
      if len(set_ty)==2 and flag:
        ...
        ax2.plot(sx,sy,label=label,color=color_line[i%9],linewidth=.5,marker=markers[(6-i)%6],markersize=.1)
        #ax2.plot(sx,sy,label=label)
        #ax2.scatter(sx, sy,5)
        ax2.legend(prop=font)
        ax2.legend(bbox_to_anchor=(1.05,1.25), loc='upper center', prop=font)   
        #ax2.xaxis.set_major_locator(locator)

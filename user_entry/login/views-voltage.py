...
def add(request):
  ...
  if chart_type == 'bar': 
    ...
    color = 'black'
    ax1.set_xlabel('Time (hh:mm:ss)',fontsize=8.5,color=color)
    
    ax1.set_ylabel(l1, fontsize=8.5,color=color)
    ...
    if len(set_ty)>=3:
      ...
      if l3 in 'voltage':
        l3='Voltage (V)'
      ax2.set_ylabel(l2, fontsize=8.5,color=color)  # we already handled the x-label with ax1
    #ax2.plot(t, data2, color=color)
      ax3.set_ylabel(l3, fontsize=8.5,color=color)  # we already handled the x-label with ax1
    ...
    for row in range(0,len(sensor_id)):
      ...
      if not(flag):      
        #amritaformatter = mdates.DateFormatter("%d-%m-%y")	
      ...
      if len(set_ty)>=2 and flag and  flag1:
        ...
        ax3.plot(tx,ty,label=label,color=color_line[i%10],linewidth=.3,marker=markers[(6-i)%6],markersize=.1)

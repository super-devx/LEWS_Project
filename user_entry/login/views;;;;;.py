...
def add(request):
  ...
  if chart_type == 'bar': 
    ...
    #fig, ax1 = plt.subplots(figsize=(3.4,3.4))
    fig, ax1 = plt.subplots()
    color = 'black'
    ax1.set_xlabel('Time',fontsize=12)
    
    ax1.set_ylabel(l1, fontsize=12,color=color)
    ...
    if len(set_ty)>=3:
      ...
      if l3 in 'voltage':
        l3='Voltage (V)'
      ax2.set_ylabel(l2, fontsize=12,color=color)  # we already handled the x-label with ax1
    #ax2.plot(t, data2, color=color)
      ax3.set_ylabel(l3, fontsize=12,color=color)  # we already handled the x-label with ax1
    ...
    for row in range(0,len(sensor_id)):
      ...
      if not(flag):      
        ...
        #ax1.legend(bbox_to_anchor=(.52,1.14), loc='upper center', prop=font)
      ...
      if len(set_ty)>=2 and flag and ab:
        ...
        if len(sx)>0:
          ax2.plot(sx,sy,label=label,color=color_line[i%10],linewidth=.5,marker=markers[(6-i)%6],markersize=.1)
        #ax2.plot(sx,sy,label=label)
        #ax2.scatter(sx, sy,5)
        ax2.legend(prop=font)
        #helpax2.legend(bbox_to_anchor=(.50,1.25), loc='upper center', prop=font,ncol=2)   
        #ax2.legend(bbox_to_anchor=(0.17,1.14), loc='upper center', prop=font,ncol=1)
      ...
      if len(set_ty)>=2 and flag and  flag1:
        ...
        ax3.plot(tx,ty,label=label,color=color_line[i%10],linewidth=.5,marker=markers[(6-i)%6],markersize=.1)
        
        print('error1ax3')
        ax3.legend(prop=font)
        #ax3.legend(bbox_to_anchor=(.845,1.14), loc='upper center', prop=font)   
